...
const DatePicker = ( { selectedValue="", handleSetValue=function(){}, dateType="" } ) => {
  ...
  return (
    <div className="w-full h-full relative">
      <div className="w-full flex-col justify-start items-center gap-2">
        <label
          htmlFor={`input-date-${dateType}`}
          className="w-full text-slate-200"
        >
           {dateType === "departure" ? "Fecha Salida" : "Fecha Regreso" } 
        </label>
        <div className="w-full flex justify-between items-center gap-4 mt-2">
          <input
            id={`input-date-${dateType}`}
            type="date"
